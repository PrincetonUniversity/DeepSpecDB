void insert (node_t **r, int k, void *v){
		...
		pn->n = *r;
		$\color{blue} \left\{\begin{array}{l} \texttt{pn} \mapsto (\texttt{\_}, \texttt{r}) \ast  \nodeboxrep(\texttt{r}) \end{array}\right\}$
		acquire(pn->n->lock);
		$\color{blue} \left\{\begin{array}{l}  \texttt{pn} \mapsto (\texttt{\_}, \texttt{r}) \ast  \nodeboxrep(\texttt{r}) \ast \mathsf{R}(\texttt{r}) \end{array}\right\}$
		if (traverse(pn, k) == 0){
		$\color{blue} \left\{\begin{array}{l}  \exists\ n'.\ \texttt{pn} \mapsto (n', n') \ast \nodeboxrep(n') \ast \mathsf{node\_contents}(n', (\texttt{k}, \mathsf{v'}), \mathsf{range'}) \end{array}\right\}$
				pn->p->t->value = v;
