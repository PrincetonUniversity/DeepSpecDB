...
\infp (\texttt{r}) \end{array}\right\} \Rrightarrow \left\{\begin{array}{l} \mathsf{traverse\_inv} \end{array}\right\}$
		for( ; ; ){ 			$\color{blue} \left\{\begin{array}{l} \mathsf{traverse\_inv} \end{array}\right\} \triangleq \left\{\begin{array}{l} 
		 ...
		 \end{array}\right\}$
				...
				if (inRange(pn, k) == 1){
						$\color{blue} \left\{\begin{array}{l} \texttt{k} \in \mathsf{range} \ast \texttt{pn} \mapsto (n', n') \ast \infp (n') \ast \mathsf{node\_contents}(n', \mathsf{c}, \mathsf{range}) \ast \infp (\texttt{r}) \end{array}\right\}$
						if (pn->p->t == NULL)
								return 1;
								...
								$\color{blue} \left\{\begin{array}{l} \texttt{k} \in \mathsf{range} \ast \texttt{pn} \mapsto (n', n') \ast \infp (n') \ast \mathsf{node\_contents}(n', (\mathsf{k'}, \mathsf{v'}), \mathsf{range}) \ast \infp (\texttt{r}) \end{array}\right\}$
								int b = findNext(pn, k);
								$\color{blue} \left\{\begin{array}{l} \exists \  n''. \ \texttt{pn} \mapsto (n', n'') \ast \infp (n') \ast  \texttt{k} \in \mathsf{range} \ast \mathsf{node\_contents}(n', (\mathsf{k'}, \mathsf{v'}), \mathsf{range})\ \ast \\\infp (\texttt{r}) \ast
						((\texttt{b} = 0 \ast \mathsf{k'} = \mathsf{k} \ast n'' = n')  \lor 						 (\texttt{b} = 1  \ast \infp (n'')))   \end{array}\right\}$
								if (b == 0){
										$\color{blue} \left\{\begin{array}{l} 
								\texttt{pn} \mapsto (n', n') \ast \mathsf{k' = \texttt{k}} \ast \cdots    \end{array}\right\}$
										return 0;
										$\color{violet} \left\{\begin{array}{l} \texttt{pn} \mapsto (n', n') \ast \infp (n') \ast  \texttt{k} \in \mathsf{range'} \ast \mathsf{node\_contents}(n', (\texttt{k}, \mathsf{v'}), \mathsf{range})   \end{array}\right\}$
								}
								else
										$\color{blue} \left\{\begin{array}{l} \texttt{pn} \mapsto (n', n'') \ast \infp (n') \ast  \texttt{k} \in \mathsf{range} \ast \mathsf{node\_contents}(n',(\mathsf{k'}, \mathsf{v'}), \mathsf{range})\ \ast\\ \infp (n'') \ast \infp (\texttt{r})   \end{array}\right\}$
										release(pn->p->lock);
