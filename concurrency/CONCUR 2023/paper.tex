...
\begin{align*}
	&\mathsf{node\_contents}(n, \mathsf{c}, \mathsf{range}) \triangleq \mathsf{ghost\_node(n, \mathsf{c}, range)} \ \ast \ \mathsf{node\_data}(n, \mathsf{c})\\
	&\mathsf{R}(n) \triangleq \exists\ \mathsf{c}, \mathsf{range}.\ \mathsf{node\_contents}(n, \mathsf{c}, \mathsf{range})\end{align*}
where the definition of $\mathsf{node\_data}$ is supplied by the target data structure. The contents $\mathsf{c}$ of a node can be either a key-value pair $(\texttt{k}, \mathsf{v})$, or the empty contents $\cdot$ (used for nodes that have been allocated but not yet assigned keys).
...
\begin{align*}\mathsf{traverse\_inv}(\texttt{pn}) \triangleq\ \exists \ p, n, \mathsf{c}, \mathsf{range}.\ &\texttt{pn} \mapsto (p, n) \ast \texttt{k} \in \mathsf{range}\ \ast \  \nodeboxrep(n)  \ \ast \\  &\mathsf{node\_contents}(n, \mathsf{c}, \mathsf{range})
\end{align*}
