...
Next, we release (line 11) the node \texttt{n} and return it to the shared state. Subsequently, we demonstrate the postcondition of the \texttt{insert} function to complete the proof.
...
We present the mechanized formal verification of the correctness of these templates, which are implemented in C and verified with the Verified Software Toolchain (VST). We employ concurrent separation logic (CSL) to analyze the behavior of the C code and incorporate Iris-style logical atomicity within VST.

By utilizing VST, we can create programs in standard C, automatically generate abstract syntax trees (ASTs) for them in Coq, and subsequently state and interactively prove specifications for these programs with the support of automated tactics. All proofs are formalized in Coq using VST and can be found online at
