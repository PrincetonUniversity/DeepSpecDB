...
\item To the best of our knowledge, this is the first mechanized verification of a template approach to concurrent data structure implementations in a real programming language, and its first application to data structures not written specifically as case studies.
...
The logically atomic specification for the lock-coupling template's \lstinline{traverse} function is shown in Figure \ref{fig:traverse_lock}. \wm{Note that this isn't the same as give-up, and that reflects the original paper. We might be able to put a wrapper function that acquires the initial lock for lock-coupling?}
...
Our target data structure is a binary search tree (BST) implemented in C. We discuss the verification of \texttt{insert} in detail for each template; the \texttt{lookup} proofs follow similar reasoning. As for \texttt{delete}, there is a mismatch between the template approach and BST deletion, which we discuss in Section \ref{internal_reorganize}. The complete proofs in VST can be found online at \href{https://github.com/PrincetonUniversity/DeepSpecDB/tree/master/concurrency/templates}{\color{blue}{https://github.com/PrincetonUniversity/DeepSpecDB/tree/master/concurrency/templates}}; we discuss the overall development in Section \ref{proof_mech}.

\wm{Should we have a separate section where we introduce findNext and insertOp before we get into specific templates?}
...
\begin{figure}[h]
	...
	\end{mathpar}
	\caption{Specification of \texttt{insertOp} for the lock-coupling template \wm{Is this different for the give-up template? Why?}}
...
\wm{I think we should try to explain what those changes actually are.}
...
\wm{The proof for give-up is slightly different from the lock-coupling version, because the lock-coupling one needs to acquire an extra lock to satisfy the precondition of its traverse.}
...
%future work: how generically can we implement the templates, and then be able to apply them to real existing data structure implementations?
%say something about the fact that BST existed before this work, rather than being written to the template?
