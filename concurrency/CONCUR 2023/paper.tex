...
\begin{figure}[ht]
	...
	\begin{mathpar}
		...
		{\color{blue}
			...
			\nodeboxrep(\texttt{pn->n})  \ \ast \ \mathsf{R}(\texttt{pn->n}) \ \big| \ \treerep\ m \
			\right\rangle
		}
		\\ 
		\texttt{traverse(pn, k)} 
...
\begin{mathpar}\mathsf{traverse\_inv}(\texttt{pn}) \triangleq\ \exists \ \texttt{n'}.\ \texttt{k} \in \mathsf{range}(\texttt{n'})\ \land \ \nodeboxrep(\texttt{pn->n'})  \ \ast \  \mathsf{R}(\texttt{pn->n'})
\end{mathpar}\wm{Where does \lstinline{range} come from here?}

\newcommand\lstsetnumber[1]{\gdef\thelstnumber{#1}}
\begin{figure}[ht]
	$\color{blue}\left\langle\begin{array}{l} \nodeboxrep(b,g\_root)\ *\ \mathsf{atomic\_shift}(P_p,E_i,E_o,Q_p,Q)\end{array}\right\rangle$
	\begin{lstlisting}[language = C, style=myStyle, mathescape=true]
		int traverse(pn *pn, int k){
			  int flag = 1; 			$\left\{\begin{array}{l} 		\ghost{\mathsf{my\_half}((-\infty,+\infty),info)}{g\_root}\ *\ R\ b\ *\ \mathsf{lock\_inv}(l,lsh2,R')\ *\ \\
		  		...
		  		for( ; ; ){ 			$\left\{\begin{array}{l} lookup\_inv \end{array}\right\} \triangleq \left\{\begin{array}{l}(x\in range)\land \ghost{\mathsf{my\_half}(range,info)}{g\_current}*\ R\ np\ *\\\mathsf{lock\_inv}(lock,lsh2,R')\ *\ \nodeboxrep(b,g\_root)\ *\ \mathsf{atomic\_shift}(P_p,E_i,E_o,Q_p,Q)\end{array}\right\}$
		  			pn->p = pn->n;
		  			if (pn->p->t == NULL)
		  					break;
		  			else{
  				...
  				return flag;
  		}  $\lstsetnumber{}$\end{lstlisting}
$\color{blue}\left\langle\begin{array}{l} \nodeboxrep(b,g\_root)\ *\ \mathsf{atomic\_shift}(P_p,E_i,E_o,Q_p,Q)\end{array}\right\rangle$
\caption{proof of \texttt{traverse}}
\end{figure}
	...
	\begin{mathpar}
		...
		{\color{blue}
			...
			\begin{array}{c}
				...
				\\ \texttt{(if res then } \texttt{(min < k < max)} \texttt{ else } \texttt{(k} \leq \texttt{min} \lor \texttt{k} \geq \texttt{max))}
			\end{array}
			\right\}
		}
	\end{mathpar}
