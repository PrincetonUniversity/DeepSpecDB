...
\than{I put an explanation for $\mathsf{node\_contents}(\texttt{n}, \cdot , \mathsf{range})$ here }

The expression $\mathsf{node\_contents}(\texttt{n}, \cdot , \mathsf{range})$  signifies that \texttt{n} points are being directed towards an empty leaf node. On the other hand, the expression $\mathsf{node\_contents}(\texttt{n}, (\mathsf{k}, \mathsf{v}), \mathsf{range})$ denotes that \texttt{n} points are directed towards a node, which houses the key $\mathsf{k}$ and its corresponding value $\mathsf{v}$.
...
\begin{mathpar} \mathsf{traverse\_inv}(\texttt{pn}) \triangleq \ \exists \ \texttt{p r n'}.\ \texttt{pn} \mapsto (\texttt{p, n'}) \ast \ \infp (\texttt{r}) \ast \infp (\texttt{n'})
\end{mathpar}
