...
\begin{mathpar}\mathsf{traverse\_inv}(\texttt{pn}) \triangleq\ \exists \ \texttt{n'}.\ \texttt{pn} \mapsto (\texttt{p, n'}) \ast \texttt{k} \in \mathsf{range}(\texttt{n'})\ \ast \  \nodeboxrep(\texttt{n'})  \ \ast \  \mathsf{R}(\texttt{n'}) \ \ast \ \treerep\ m
...
\begin{figure}[h]
	...
	\begin{mathpar}
		...
		{\color{blue}
			\left\langle \mathit{res}. \ \exists \ \texttt{n'}.
			\begin{array}{c}
				\texttt{pn} \mapsto (\texttt{p, n}) \ \ast \ (\mathit{res} \leftrightarrow (\texttt{pn->n'->t} = \texttt{NULL}))  \ \ast \ 
