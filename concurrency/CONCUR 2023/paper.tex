...
To implement hand-over-hand locking, which guarantees that at least one lock is maintained during traversal between nodes (i.e., the lock for the next node is obtained before releasing the lock for the current node), the \lstinline{pn} struct contain two nodes of type \lstinline{tree_t}: the \lstinline{p} field, denoting the current node, and the \lstinline{n} field, representing the next node.
...
To utilize the template methodology for concurrent data structure implementations in C, we implement the \lstinline{traverse} function in C, as depicted in Figure \ref{traverse_lock_a}. The \lstinline{pn} struct include two fields: \lstinline{p} and \lstinline{n}, which represent the current and next node, respectively. Additionally, we require a target key \lstinline{k}, and a dummy node's value \lstinline{value} (which we call dummy because it is not actually used in \lstinline{traverse}, but we include it to maintain consistent function signatures with other implementations). Our version of the \lstinline{traverse} function is similar to the one written in an ML-like language, as it also relies on a helper function called \lstinline{findnext}. The purpose of \lstinline{findnext} is to determine the next node to be visited during the traversal. In our implementation, the \lstinline{traverse} function stops when it reaches the base case, which occurs when the current node is \lstinline{NULL}. Alternatively, it terminates when \lstinline{findnext} fails to find a next node that the key value $k$ contains in the set of key values. The \lstinline{traverse} function returns a boolean value that indicates whether it has successfully found the next node during traversal. If it fails to do so, it returns false, allowing other functions to make decisions based on this result. For example, in a binary search tree, a node with a value of 20 would have a left child with a set of key values consisting of all values less than 20, and a right child with one greater than 20. If we want to insert a new node with the target key $k$ into the tree, we can use the \lstinline{traverse} function to navigate the tree. When \lstinline{traverse} fails to find the next node (either to the left or right), it means that it has encountered a node with a target key $k$ that is the same as the value of the node. At this point, we simply need to update the value of that node to \lstinline{value}. When \lstinline{findnext} reaches a new node, it acquires a lock for that node, as is the nature of hand-over-hand locking. It then releases the lock for the current node, ensuring that the link between the two nodes is not removed or rearranged while we traverse it.
...
Similar to the lock-coupling template algorithm, the give-up template algorithm utilizes a helper function named \lstinline{inRange} in addition to the \lstinline{findNext} function discussed earlier in the lock-coupling section. The role of the \lstinline{inRange} function is to check whether the key value $k$ belongs to the range of node $n$. If the key value is outside the node's range, the search is abandoned, and the thread is restarted.
