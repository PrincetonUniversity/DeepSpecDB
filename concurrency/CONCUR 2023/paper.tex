...
\begin{mathpar}
	...
	{\color{blue}
		\left\{ 
		\begin{array}{c}
			\texttt{pn}\mapsto\texttt{n} \ \ast \texttt{n}\mapsto\texttt{t} \ \ast \ \texttt{n}\mapsto\texttt{min} \ \ast \ \texttt{n->max}
		\end{array}
		\right\}
	}
	\\ 
	...
	{\color{blue}
		...
		\begin{array}{c}
			 ...
			 \ \ast  \ \texttt{(if res then } \texttt{(min < x} \land \texttt{x < max)} \land \texttt{(x} \leq \texttt{min} \lor \texttt{x} \geq \texttt{max))}
		\end{array}
		\right\}
	}
\end{mathpar}
...
\begin{figure}
	...
	\begin{array}{c}
		\ \texttt{pn}\mapsto\texttt{n} \ \ast \ \texttt{n}\mapsto\texttt{t'} \ \ast \ \texttt{t'}\mapsto \texttt{(k, v, l, r)} \\ 
		\ \ast \ \texttt{l} \mapsto \texttt{(NULL, lk1, (min, x))} \ \ast \ \texttt{r} \mapsto \texttt{(NULL, lk2, (x, max))}
	\end{array}
	\right\}
	}
\end{mathpar}
