...
The particulars of $\nodeboxrep$ and $\treerep$ might differ between implementations, but at their core, $\nodeboxrep$ encompasses the resources tied to each client thread, while $\treerep$ acts as an abstract representation containing ghost state that connects the data structure's concrete state to its abstract counterpart and is treated as an atomically accessed shared resource. Specifically, $\nodeboxrep(\texttt{pn->n})$ functions as a pointer to both the node's lock and lock-invariant assertions, representing a protected resource through an arbitrary separation logic predicate. Acquiring this lock enables a thread to access further elements within the data structure. As previously mentioned, the \lstinline{pn} struct comprises two nodes of type \lstinline{node_t}: the $\texttt{p}$ field, which signifies the parent node, and the $\texttt{n}$ field, representing the current node. 

We use the assertion $\ell \lockvar \texttt{R}$ to associate a resource invariant \texttt{R} with a lock location $\ell$. Thus, formally, we can write $\nodeboxrep \texttt{(pn->n)} = \texttt{pn->n->lock} \lockvar \texttt{R(pn->n)}$.  Remember that in the lock-coupling style, each thread consistently maintains at least one lock during traversal between nodes, ensuring other threads cannot disrupt the traversal or invalidate the ongoing search. Thus, before invoking the \texttt{traverse} function, the thread must acquire a lock for the current node $\texttt{pn->n}$ and obtain the resource protected by the lock. This resource, denoted as $\texttt{R(pn->n)}$, is parameterized by a node pointer $\texttt{pn->n}$ and describes the concrete information about the node. 

In the specification, the local pre-condition encompasses $\nodeboxrep(\texttt{pn->n})$ and resource $\texttt{R(pn->n)}$. Upon operating on the entire search structure, the \texttt{traverse} function yields a subsequent node \texttt{n'}, where \texttt{k} falls within the range of node \texttt{n'}.   The local post-condition is expected to contain $\nodeboxrep(\texttt{pn->n'})$, resource $\texttt{R(pn->n)}$, and a Boolean \texttt{res} that indicates whether \texttt{pn->n'->t} is \texttt{NULL} or not.

%The lock invariant $\lockinv$ predicate asserts that a lock exists to protect the resources.

%The specification states that if the node \texttt{n} is within the footprint of the search structure, then the \texttt{traverse} function operates on the entire search structure and returns a subsequent node \texttt{n'}, where \texttt{k} falls within the range of node \texttt{n'}.
