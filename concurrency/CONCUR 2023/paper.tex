...
%% C implementation 
To implement the template algorithm, it is crucial to keep track of the lower and upper bounds of the keys present in the subtree rooted at the current node. Therefore, we have incorporated two new fields, \lstinline{min} and \lstinline{max}, into each node of type \lstinline{tree_t} in our C implementation. These fields respectively store the lower and upper bounds of the keys. When a thread arrives at a node in search of $x$, it check if $x$ falls within the range of that node.
