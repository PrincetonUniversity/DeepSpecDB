...
\begin{align*} &\mathsf{traverse\_inv}() \triangleq\ \exists\ (\mathsf{k} \in \mathsf{range})\land \mathsf{R(pn->p)} \ast  \nodeboxrep(\mathsf{pn})\ \ast \cdots 
\end{align*}
...
\begin{align*} 
	&\mathsf{traverse\_inv\_1}() \triangleq\ \exists\ (\mathsf{k} \in \mathsf{range})\land \mathsf{R(pn->p)} \ast  \nodeboxrep(\mathsf{pn})\ \ast \cdots 
	\\
	& \mathsf{traverse\_inv\_2}() \triangleq\ 
\end{align*}
