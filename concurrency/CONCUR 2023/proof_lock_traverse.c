int traverse(pn *pn, int k){
		...
		for( ; ; ){ 			$\color{blue} \left\{\begin{array}{l} \mathsf{traverse\_inv} \end{array}\right\} $
				pn->p = pn->n;
				$\color{blue} \left\{\begin{array}{l} \texttt{pn} \mapsto (\texttt{n', n'}) \ast  \nodeboxrep(\texttt{n'}) \ast  \texttt{k} \in \mathsf{range'} \ast \mathsf{node\_contents}(\mathsf{k'}, \mathsf{v'}, \mathsf{range'})\end{array}\right\}$
				if (pn->p->t == NULL)
						return 1;						$\color{violet} \left\{\begin{array}{l} \texttt{pn} \mapsto (\texttt{n', n'}) \ast \texttt{n'->t} = \texttt{NULL}  \ast \nodeboxrep(\texttt{n'}) \ast  \texttt{k} \in \mathsf{range'} \ast \mathsf{node\_contents}(\mathsf{k'}, \mathsf{v'}, \mathsf{range'}) \end{array}\right\}$
				else{
						int b = findNext(pn, k);
