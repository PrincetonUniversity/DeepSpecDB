...
\LB{In contrast to all previous wroks, we our work is on full C implementations of binary search trees rather than pseudo code. This is enabled by recently added incorporation of (slightly limited) Iris-style ghost state and invariants (and Iris prof mode), and of TaDa-style atomic specs in VST (citation to William's technical report?)}\wm{It's currently a rejected ITP paper rather than a technical report, sadly.}