...
\begin{abstract}
Modern concurrent separation logic techniques have been very effective at proving that lock-free data structures implement atomic specifications: they perform the same operations as sequential data structures, and do not expose any intermediate states to other threads. In this paper, we describe a method of proving the same kinds of specifications for lock-based data structures, with both coarse-grained and fine-grained locking patterns. We begin from the traditional separation logic specifications of locks and lock invariants, and combine them with a ghost state pattern that connects the critical section of a lock with the linearization point of a data structure operation. For fine-grained data structures, this allows us to decompose the proofs into a proof that a lock-based program performs some local changes to ghost state, and a proof that these local changes correctly implement an update to the global state of the data structure. Our proof technique is implemented in the Verified Software Toolchain and applied to C implementations of lock-based concurrent data structures.
...
\subsubsection{Acknowledgements} I want to thank Zhen Zheng and Ralf Jung, ...
