...
Inductive IsEmptyGhostNode (range : number * number * option ghost_info ) :  (@ghost_tree val) -> (number * number) -> Prop :=
 ...
 Lemma public_range_incl : forall g (a b: G),
     my_half g gsh1 a * public_half g b |-- !!(range_inclusion a.1 b.1 = true).
 Proof.
   intros. unfold my_half, public_half. eapply derives_trans; [apply ref_sub|].
   apply prop_left; intro; apply prop_right.
   rewrite if_false in H; auto with share. destruct H. hnf in H. destruct H.
   hnf in H. eapply merge_range_incl; eauto.
 Qed.
...
Lemma in_tree_left_range:
  ∀ (B: Type) (b: tree -> B -> mpred) (Q : B → mpred) (x x0: Z) (g g_root : gname)
    ...
    (!! (exists ba, less_than_equal ba r.1.1 = true /\
                    range_inclusion a.1 (ba, Finite_Integer x0) = true) &&
     (in_tree g g_in * my_half ga gsh1 a)).
Proof.
  ...
  - Intros r0 x1 v0 ga0 gb0 i1 i2.
    ...
    Intros. rewrite if_false in H5. 2: auto with share.
    destruct H5. inv H5. simpl fst in *. simpl snd in *. rewrite H0 in H7.
    inv H7. 2: inv H9. clear H8. sep_apply public_range_incl. Intros. simpl fst in H3.
    rewrite <- wand_sepcon_adjoint.
    eapply derives_trans; [|apply ghost_seplog.bupd_intro]. entailer!.
    + exists r0.1. split; auto. hnf in H6. symmetry in H6. clear -H6.
      apply merge_range_incl in H6. destruct r. simpl in *. destruct g, r0.
      simpl in H6. apply andb_true_iff in H6. simpl. now destruct H6.
...
Lemma in_tree_right_range:
  ∀ (B: Type) (b: tree -> B -> mpred) (Q : B → mpred) (x x0: Z) (g g_root : gname)
    ...
    |-- atomic_shift (λ BST : tree, !! sorted_tree BST && tree_rep2 g g_root BST) ∅ ⊤
    b Q * my_half g_in gsh1 r *
    (!! (a.1 = (Finite_Integer x0, r.1.2)) && (in_tree g g_in * my_half gb gsh1 a)).
Proof.
 intros. rewrite sepcon_assoc. apply sync_rollback. intros t.
  unfold tree_rep2 at 1. Intros tg.
  assert_PROP (Ensembles.In _ (find_ghost_set tg g_root) g_in). {
...
Lemma node_info_join_mid: forall (r0 r1 r2: node_info),
    sepalg.join r0 r1 r2 -> exists r3, sepalg.join r0 r3 r0 /\ sepalg.join r2 r3 r2.
Proof.
  (* Set Printing Implicit. *)
  intros. destruct r0 as [rg0 r0]. destruct r1 as [rg1 r1]. destruct r2 as [rg2 r2].
  hnf in H. simpl in H. destruct H. exists (rg0, @None (option ghost_info)).
  split; hnf; simpl; split; hnf; [|constructor| |constructor].
  - now rewrite merge_self.
  - symmetry. rewrite merge_comm. apply merge_again with rg1. hnf in H. easy.
Qed.
...
Proof.
  ...
  - (* loop body *)
    ...
    + forward. (* _tgt = (_p -> _left); *)
      ...
      * destruct H9 as [ba []]. destruct a as [rangea a]. simpl fst in *.
        simpl snd in *. Exists ((((tpa, pa), (ba, Finite_Integer x0, a)), ga), locka).
        simpl fst. simpl snd. sep_apply (range_incl_tree_rep_R tpa _ _ a g H10).
        entailer!.
        -- simpl. rewrite andb_true_iff. clear -H2 H7 H9. split.
    ...
    + forward_if. (* if (_y < _x) { *)
      ...
      * forward. (* _v = (_p -> _value); *)
        ...
        viewshift_SEP 0 (EX y, Q y *
          ...
          eapply derives_trans; [|apply ghost_seplog.bupd_intro].
          destruct H11 as [r1 ?]. destruct (node_info_join_mid _ _ _ H11) as [m [? ?]].
          Exists m r r0. apply andp_right. 1: now apply prop_right.
          rewrite <- wand_sepcon_adjoint.
          eapply derives_trans; [|apply ghost_seplog.bupd_intro].
  ...
  - subst tp0. unfold tree_rep_R. simpl. Intros.
    ...
    viewshift_SEP 0 (EX y, Q y *
      ...
      eapply derives_trans; [|apply ghost_seplog.bupd_intro].
      destruct H8 as [r1 ?]. destruct (node_info_join_mid _ _ _ H8) as [mid [? ?]].
      Exists mid r r0. apply andp_right. 1: now apply prop_right.
      rewrite <- wand_sepcon_adjoint.
      eapply derives_trans; [|apply ghost_seplog.bupd_intro].
            ...
            intros. iIntros "H". iDestruct "H" as "[H1 H2]". iDestruct "H2" as "[% H2]". 
             ...
             iExists (n1,n2, Some o0). iFrame. iPoseProof ( bi.and_elim_l with "H2") as "H3".  iPoseProof ( bi.and_elim_l with "H3") as "Hnew". iIntros "%". iMod "Hnew".   iDestruct "Hnew" as (g1 g2) "H". iAlways.  iExists (n1,n2, Some (Some(x,v,g1,g2))). 
              ...
              match goal with |-context[(|==> ?P)%logic] => change ((|==> P)%logic) with ((|==> P)%I) end. iSplit.
               { iPureIntro. intros. (*  hnf in *. simpl in *. destruct H8. split. auto. subst o. destruct b0 as [r i]. destruct r as [n3 n4]. 
               rewrite if_false in a. inversion a as [ x1 Hr].  apply node_info_join_Some in Hr. simpl in Hr. rewrite Hr in H9. apply sepalg.join_unit2. simpl in *. inv H9. auto. inv H12. auto. apply gsh1_not_Tsh. *)
                destruct b0 as [r i]. destruct r as [n3 n4].   hnf . simpl. split. apply sepalg_range_inclusion in H8. simpl in *. hnf in *. unfold merge_range. inv H8. 
             ...
             iIntros "(H1 & H2)". instantiate (1 := x). instantiate (1:= v).  iSpecialize ("H" with "[H2]"). iFrame. iSplit;auto.
              { iPureIntro. split. rewrite if_false in a. inversion a as [ x1 Hr]. subst o.  apply node_info_join_Some in Hr. simpl in Hr. inv Hr;auto. apply gsh1_not_Tsh.
               ...
               iDestruct "H" as "(((((H2 & H3) & H4) & H5) & H6) & H7 )".  rewrite <- (my_half_join (P :=  node_ghost) gsh1 gsh2 Tsh  (n1, Finite_Integer x, Some None)  (n1, Finite_Integer x, Some None) (n1, Finite_Integer x, Some None) g1).  
              rewrite <- (my_half_join (P :=  node_ghost) gsh1 gsh2 Tsh  ( Finite_Integer x, n2, Some None)  ( Finite_Integer x, n2, Some None) ( Finite_Integer x, n2, Some None) g2). iDestruct "H3" as "[H3 H8]". iDestruct "H4" as "[H4 H9]". 
              iMod (own_dealloc with "H8") as "_". iMod (own_dealloc with "H9") as "_".  iModIntro. normalize. iExists g1. normalize.  iExists g2. apply (insert_sorted x v) in H7. iSplit. auto. iFrame. iExists n1. iExists n2. iFrame.
               iSplit.  iPureIntro.  (* rewrite if_false in a. inversion a as [ x1 Hr]. subst o.  apply node_info_join_Some in Hr. simpl in Hr. inv Hr;auto. apply gsh1_not_Tsh. *)
               rewrite if_false in a. inversion a as [ x1 Hr]. subst o.  apply sepalg_range_inclusion in Hr. destruct Hr. simpl in H4. apply andb_prop in H4. unfold check_key_exist' in *. apply andb_prop in H3. destruct H3. apply andb_true_intro.
               destruct H4.  split. apply less_than_equal_less_than_transitivity with (b := n);auto. apply less_than_less_than_equal_transitivity with (b := n0);auto. apply gsh1_not_Tsh. auto. done. apply gsh1_gsh2_join. hnf;simpl. split. hnf. rewrite merge_self;auto.
                admit. apply gsh1_not_bot. apply gsh2_not_bot. apply gsh1_gsh2_join. admit. apply gsh1_not_bot. apply gsh2_not_bot.
              }      
      Intros g1 g2 n1 n2. 
      Typeclasses eauto:= 2.
    ...
    + (* else clause *)
      ...
      - (* Inner if, then clause: x<k *)
        ...
        unfold node_lock_inv at 2. rewrite selflock_eq. unfold node_lock_inv_pred at 1.  unfold sync_inv at 1.  Intros a.
         rewrite node_rep_def. Intros tp1. gather_SEP (atomic_shift _ _ _ _ _) (my_half g_in _ _) (in_tree g g_in) (my_half ga lsh1 a). 
