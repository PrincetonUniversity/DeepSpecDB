...
int GET_FLAG(struct operation *ptr)
{
    return ptr->type;
}
...
void helpChildCAS(operation *op, tree *dest, int thread_num)
{
    //add_to_hp_list(thread_num, op->expected);
    child_cas_operation *cas_op = (child_cas_operation *)op->op;
    atom_ptr *address = cas_op->is_left ? dest->left : dest->right;
    void *tp = cas_op->expected;
    if (atomic_CAS_ptr(address, &tp, cas_op->update))
    {
    ...
    struct operation *new_op = (struct operation *)surely_malloc(sizeof *new_op);
    new_op->op = NULL;
    new_op->type = NONE;
    void *old = op;
    int clear = atomic_CAS_ptr(dest->op, &old, new_op);
    if (!clear)
    {
        free(new_op);
    }
...
void helpMarked(tree *pred, void *pred_op, tree *curr, int thread_num)
{
    ...
    struct operation *new_op = (struct operation *)surely_malloc(sizeof *new_op);
    new_op->op = cas_op;
    new_op->type = CHILDCAS;

    if (atomic_CAS_ptr(pred->op, &pred_op, new_op))
    {
        helpChildCAS(new_op, pred, thread_num);
    }
    else
    {
        free(cas_op);
...
int helpRelocate(operation *op, tree *pred, void *pred_op, tree *curr, int thread_num)
{
    
    relocate_operation *reloc_op = (relocate_operation *)op->op;
    int seen_state = atom_load(reloc_op->state);
    tree *real_dest = atomic_load_ptr(reloc_op->dest);
    //add_to_hp_list(thread_num, real_dest);
    if (seen_state == ONGOING)
    {
        operation *old_op = atomic_load_ptr(real_dest->op);
        int success = atomic_CAS_ptr(real_dest->op, &reloc_op->dest_op, op);
        if (success || (old_op == op))
        {
            int i = ONGOING;
            atom_CAS(reloc_op->state, &i, SUCCESSFUL);
            seen_state = SUCCESSFUL;
        }
        else
        {
            int i = ONGOING;
            seen_state = atom_load(reloc_op->state);
            atom_CAS(reloc_op->state, &i, FAILED);
        }
    }

    if (seen_state == SUCCESSFUL)
    {
        real_dest->key = reloc_op->replace_key;
        atomic_store_ptr(real_dest->value, reloc_op->replace_value);
        struct operation *new_op = (struct operation *)surely_malloc(sizeof *new_op);
        new_op->op = NULL;
        new_op->type = NONE;
        void *old = op;
        int clear = atomic_CAS_ptr(real_dest->op, &old, new_op);
        if (!clear)
        {
            free(new_op);
        }
    ...
    struct operation *new_op = (struct operation *)surely_malloc(sizeof *new_op);
    new_op->op = NULL;
    new_op->type = result ? MARK : NONE;
    void *old = op;
    int clear = atomic_CAS_ptr(curr->op, &old, new_op);
...
void help(tree *pred, void *pred_op, tree *curr, void *curr_op, int thread_num)
{
    ...
    {
        helpChildCAS(((operation *)curr_op), curr, thread_num);
    }
    else if (GET_FLAG(curr_op) == RELOCATE)
    {
        helpRelocate(((operation *)curr_op), pred, pred_op, curr, thread_num);
    }
    ...
    for (;;)
    {
        ...
        *curr_op = atomic_load_ptr(tp->op);
        ...
        if (GET_FLAG(*curr_op) != NONE)
        {
            ...
            {
                helpChildCAS((*curr_op), tp, thread_num);
                continue;
            }
            else
            {
        ...
        while (!IS_NULL(tp_next) && tp_next != NULL)
        {
            ...
            *curr = next;
            //add_to_hp_list(thread_num, tp);
            *curr_op = atomic_load_ptr(tp_next->op);
            if (GET_FLAG(*curr_op) != NONE)
            {
                help(atomic_load_ptr(*pred), *pred_op, tp_next, *curr_op, thread_num);
                break;
            }

            curr_key = tp_next->key;
            ...
            {
                result = FOUND;
                *v = atomic_load_ptr(tp_next->value);
                break;
            }
        }
        return result;
...
void add(int key, void *value, int thread_num)
{
    ...
    for (;;)
    {
        ...
        newNode->right = right;
        struct operation *op = (struct operation *)surely_malloc(sizeof *op);
        op->op = NULL;
        op->type = NONE;
        atom_ptr *a_op = make_atomic_ptr(op);
        newNode->op = a_op;
        ...
        if (atomic_CAS_ptr(tp->op, &curr_op, new_op))
        {

            printf("insert {%d} done\n", key);
            helpChildCAS(new_op, tp, thread_num);
            return;
        }
        else
        {
...
void delete (int key, int thread_num)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            struct operation *new_op = (struct operation *)surely_malloc(sizeof *new_op);
            new_op->op = reloc_op;
            new_op->type = RELOCATE;

            if (atomic_CAS_ptr(replace_real->op, &replace_op, new_op))
            {
                if (helpRelocate(new_op, pred_real, pred_op, replace_real, thread_num))
                {
                    return;
                }
                else
...
int main(void)
{
    ...
    root->right = right;
    struct operation *root_op = (struct operation *)surely_malloc(sizeof *root_op);
    root_op->op = NULL;
    root_op->type = NONE;
    atom_ptr *op = make_atomic_ptr(root_op);
    root->op = op;
