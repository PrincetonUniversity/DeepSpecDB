...
$$
 {\color{specblue} \{\mathsf{stack}\ s\ p\} } \ \texttt{push}(s, v)\  {\color{specblue} \{\mathsf{stack}\ (v :: s)\ p\}}$$  Its concurrent counterpart could be specified as 
\begin{mathpar}
...
\\
	{\color{specblue}\left\langle \nodeboxrep(n) \ast \mathsf{R}(n) \ |\ \treerep\ (m) \right\rangle}\ 
	 \vspace{-0.8em} \\ \texttt{release}(n\texttt{->lock})\  \vspace{-0.8em} \\ {\color{specblue}\left\langle \nodeboxrep(n) \ |\ \treerep\ (m)\right\rangle}
\end{mathpar}
