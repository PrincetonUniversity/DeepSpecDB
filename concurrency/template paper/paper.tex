...
2-by-2 grid of concurrent node implementations (bst+lock, bst+lock+range, list+lock, list+lock+range)

which pieces are data structure and which are template? (paper assumes locks and ranges are provided by data structure)

difficulty of actually implementing things this way -- even in C++, we'd have to write subclasses for BST-coupling-node, BST-giveup-node, LL-coupling-node, LL-giveup-node, and in Java those classes would have to have code in them

sketch the four approaches, describe why only one works for us?

some of the tricky pieces to separate: node creation/allocation of initial metadata, updating metadata on insert, ...

highlight the interface: which functions must a data structure provide? which functions must a template provide? which functions are written once generically, parameterized by both data structure and template?

\section{Specifying the Template Interface}
standard Hoare triples for each data structure function

atomic triples for each template function, with data structure instance as parameter

top-level specs proved once-and-for-all, with data structure and template instance as parameters
