...
Definition tree_free_spec :=
 DECLARE _tree_free
  WITH lock: lock_handle, sh : share, p: val, gv : globals, g: gname, g_root: gname
  PRE  [ tptr t_struct_tree_t ]
       PROP()
       PARAMS (p) GLOBALS (gv)
       SEP (mem_mgr gv;
...
Definition Gprog : funspecs :=
    ltac:(with_library prog [acquire_spec; release_spec; makelock_spec; (*free_atomic_spec; *)
     surely_malloc_spec; traverse_spec;  findnext_spec; tree_free_spec]).
...
Proof.
  ...
  unfold ltree; Intros.
  forward.
  forward_call acquire_inv_simple (lsh1, lock, node_lock_inv sh g p g_root lock).
  ...
  rewrite later_exp'; [ |apply (Neg_Infinity, Pos_Infinity, None)].
  Intros a.
  destruct a as (range, g_info).
  rewrite later_sepcon.
  rewrite node_rep_def.
  rewrite -> later_exp'; last admit.
  Intros tp; simpl.
  rewrite later_sepcon.
  rewrite later_sepcon.
  rewrite later_sepcon.
  ...
  forward_if (
    PROP ( )
    LOCAL (temp _p tp; temp _l (ptr_of lock); gvars gv; temp _tgp p)
    SEP (lock_inv lsh1 lock (node_lock_inv sh g p g_root lock);
        ...
        malloc_token Ews t_lock (ptr_of lock);
        lock_inv lsh2 lock (node_lock_inv sh g p g_root lock);
        mem_mgr gv; field_at lsh1 t_struct_tree_t [StructField _lock] (ptr_of lock) p)).
  unfold tree_rep_R; simpl.
  {
    ...
    forward. (* p->left *)
    forward. (* p->right *)
    forward_call (t_struct_tree, tp, gv). (* free(p) *)
