...
{ 
  ...
  db_cursor_rep: oi.(cursor) -> val-> val-> val-> mpred
}.
...
Definition put_record_spec_vp 
  (oi: OrderedIndex.index) (db: db_cursor oi): funspec :=
   WITH cur: oi.(cursor), cur_ptr:val, key:oi.(key), pkey: val, recordptr:val, record:oi.(value), gv: globals,
            e_ptr: val, t_ptr: val, ws_ptr: val
  PRE [ tptr (db_cursor_type oi db), tptr (db_key_type oi db), tptr (db_value_type oi db)]
    ...
    SEP(mem_mgr gv; oi.(value_repr) record recordptr * 
          (db_cursor_rep oi db) cur t_ptr cur_ptr ws_ptr * 
          (db_entry_rep oi db) e_ptr (oi.(key_val) key))
  POST [tvoid]
    EX newc: oi.(cursor), 
    PROP(oi.(put_record) cur key record recordptr newc)
    LOCAL()
    SEP(mem_mgr gv; (db_cursor_rep oi db) newc t_ptr cur_ptr ws_ptr * 
          (db_entry_rep oi db) e_ptr (oi.(key_val) key)).

Definition go_to_key_spec_vp
  (oi: OrderedIndex.index) (db: db_cursor oi): funspec :=
  WITH cur:oi.(cursor), cur_ptr:val, key:oi.(key), e_ptr: val, t_ptr: val, ws_ptr: val
  PRE [ tptr (db_cursor_type oi db), tptr (db_key_type oi db) ]
    PROP(oi.(go_to_key_props) cur)
    PARAMS(ws_ptr; e_ptr) GLOBALS()
    SEP((db_cursor_rep oi db) cur t_ptr cur_ptr ws_ptr * 
      (db_entry_rep oi db) e_ptr (oi.(key_val) key))
  POST [tint]
    PROP()
    LOCAL(temp ret_temp (Val.of_bool (oi.(cursor_has_next) (oi.(go_to_key) cur key))))
    SEP((db_cursor_rep oi db) (oi.(go_to_key) cur key) t_ptr cur_ptr ws_ptr * 
      (db_entry_rep oi db) e_ptr (oi.(key_val) key)).

Definition cardinality_spec_vp
  (oi: OrderedIndex.index) (db: db_cursor oi): funspec :=
  WITH cur_ptr: val, cur: oi.(cursor), t_ptr:val, ws_ptr:val
...
Definition cursor_has_next_spec_vp
  (oi: OrderedIndex.index) (db: db_cursor oi): funspec :=
  WITH cur:oi.(cursor), cur_ptr:val, t_ptr: val, ws_ptr:val
  PRE [ tptr (db_cursor_type oi db) ]
...
Definition move_to_next_spec_vp 
  ...
  POST [tint]
    ...
    SEP((db_cursor_rep oi db) (oi.(move_to_next) cur) t_ptr cur_ptr ws_ptr).

Definition move_to_first_spec_vp
  (oi: OrderedIndex.index) (db: db_cursor oi): funspec :=
  WITH gv: globals, cur_ptr: val, cur: oi.(cursor), t_ptr: val, ws_ptr: val
  PRE [ tptr (db_cursor_type oi db)]
    PROP(oi.(move_to_first_props) cur)
    PARAMS(ws_ptr) GLOBALS()
    SEP(mem_mgr gv; (db_cursor_rep oi db) cur t_ptr cur_ptr ws_ptr)
  POST [tint]
    PROP()
    LOCAL(temp ret_temp (Val.of_bool (oi.(cursor_has_next) (oi.(move_to_first) cur))))
    SEP(mem_mgr gv; (db_cursor_rep oi db) (oi.(move_to_first) cur) t_ptr cur_ptr ws_ptr).
...
Definition get_record_spec_vp
  ...
  POST [tptr (db_value_type oi db)]
    ...
    SEP((db_cursor_rep oi db) newc t_ptr cur_ptr ws_ptr).
...
Definition create_cursor_spec_vp
  ...
  POST [tptr (db_cursor_type oi db)]
    ...
    SEP(mem_mgr gv; oi.(t_repr) r t_ptr; 
      (oi.(t_repr) r t_ptr -* (db_cursor_rep oi db) (oi.(create_cursor) r) t_ptr cur_ptr ws_ptr)).
