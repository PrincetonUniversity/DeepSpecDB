...
Proof.
   intros. unfold splitnode_main_if_part2. abbreviate_semax.
    ...
    SEP (mem_mgr gv; btnode_rep (splitnode_left n e); btnode_rep (empty_node true false Last vnewnode);
         data_at Ews tentry (Vint (Int.repr (k_ k)), inr xe) pe;
         data_at Tsh (tarray tentry 16) (le_to_list (nth_first_le le fri) ++ (Vint (Int.repr (k_ k)), inr (force_val (sem_cast_pointer xe))) :: le_to_list (suble fri Fanout le) ++ ent_end) v_allEntries; le_iter_sepcon (skipn_le (insert_le le e) Middle))).
    {                           (* fri < 8 *)
      Intros.
...
SEP (mem_mgr gv; le_iter_sepcon (nth_first_le (insert_le le e) Middle);
     ...
     data_at Tsh (tarray tentry 16)
      ...
      -                         (* loop body *)
        ...
        Intros le_end.             
        forward_if.
        + assert(HINSERT: (le_to_list (nth_first_le le fri) ++ (Vint (Int.repr (k_ k)), inr (force_val (sem_cast_pointer xe))) :: le_to_list (suble fri Fanout le) ++ ent_end) = le_to_list (insert_le le e) ++ ent_end).
          { rewrite insert_fri with (fri:=fri) (key0:=ke).
            rewrite le_to_list_app.
            simpl. rewrite H5.
    ...
    assert(HINSERT: (le_to_list (nth_first_le le fri) ++ (Vint (Int.repr (k_ k)), inr xe) :: le_to_list (suble fri Fanout le) ++ ent_end) = le_to_list (insert_le le e) ++ ent_end).
    { rewrite insert_fri with (fri:=fri) (key0:=ke).
      rewrite le_to_list_app.
      simpl. rewrite H5.
      rewrite suble_skip. unfold key_repr.
      unfold n in H0. simpl in H0. rewrite <- app_assoc. simpl. reflexivity. rep_omega.
