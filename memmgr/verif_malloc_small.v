...
assert_PROP (guaranteed lens n = true <-> (Znth b lens > 0)%nat) as Hguar_eq
  by admit. (* TODO by defs mem_mgr_R and mmlist (unfortunate to unfold them though) *)
...
assert_PROP (Znth b bins <> nullval) as Hnonnull. {
  destruct Hguar_eq; assert ((Znth b lens > 0)%nat) by auto.
...
forward_if( (*! if p = null *)
  ...
  + (* after if: unroll and pop mmlist *)
    ...
    assert (Hdecr: lens' = (decr_lens lens b)). {
      ...
      f_equal.  admit. (* by arith *)
      admit. (* by reflection of Htest and contradiction *)
    }
...
forward_if( (*! if p != null *)
    EX p:val, (* EX len:Z, *)
     ...
     SEP(FRZL Otherlists; TT; 
         ...
         mmlist (bin2sizeZ b) (Znth b lens) p nullval)). 
